...
import { CommercialUnitEntity } from 'src/db/entities/commercial-unit.entity';
import { LandPlotEntity } from 'src/db/entities/land-plot.entity';
import { ResidentialUnitEntity } from 'src/db/entities/residential-unit.entity';
import { UnitFloorPlanEntity } from 'src/db/entities/unit-floor-plan.entity';
import {
...
} from 'src/db/interfaces/project.interface';
...
@Injectable()
export class ProjectUnitTypeService {
  constructor(
  ...
  async createResidentialUnit(
    ...
    try {
      ...
      const project = await this.projectManagementService.getProjectById(
        createResidentialUnitDto.project_id,
      );
      if (!project) {
        throw new NotFoundException('Project not found');
      }
      return await this.residentialUnitRepository.save(
